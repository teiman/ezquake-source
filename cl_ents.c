...
void CL_LinkPacketEntities(void){
	...
	for(pnum = 0; pnum < pack->num_entities; pnum++){
		...
		if(!(model = cl.model_precache[state->modelindex])){
			//TODO: protocol to precache after connection
			// turn this into a recuperable error
			Host_Error("CL_LinkPacketEntities: bad modelindex");
		}

		ent.model = model;
		ent.skinnum = state->skinnum; //hummm.... 
		...
		if(state->colormap >= 1 && state->colormap <= MAX_CLIENTS && (ent.model->modhint == MOD_PLAYER || (ent.renderfx & RF_PLAYERMODEL))){
		...
		//VectorCopy(cent->old_origin, cent->lerp_origin);
		...
		//Tei: turn all entities in ghost, and thats funny :D		
...
void CL_ParsePlayerinfo(void){
	...
	if(cls.mvdplayback){
		...
		//	state->weaponskin = MSG_ReadByte();//Tei 
		...
		//if (flags & PF_WEAPONSKIN) //Tei 
		//	state->weaponskin = MSG_ReadByte();
		//else
		//	state->weaponskin = 0;
		...
		if(cls.demorecording){
			...
			//if (flags & PF_WEAPONSKIN)
			//	MSG_WriteByte(&cls.demomessage, state->weaponskin);//Tei
	...
	if(!(cl.z_ext & Z_EXT_PF_SOLID)){	// the PF_SOLID bit is unsupported, use our best guess
...
static int MVD_TranslateFlags(int src){
	...
	if(src & DF_WEAPONFRAME)
		dst |= PF_WEAPONFRAME;
	//if (src & DF_WEAPONSKIN)
	//	dst |= PF_WEAPONSKIN;//Tei
	if(src & DF_MODEL)
		dst |= PF_MODEL;
